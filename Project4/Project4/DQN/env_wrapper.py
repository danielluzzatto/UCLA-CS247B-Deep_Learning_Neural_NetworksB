...
class EnvWrapper(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        ...
        for _ in range(self.initial_no_op):
            obs, reward, terminated, truncated, info = self.env.step(self.do_nothing_action)
            if terminated or truncated:
                obs, info = self.env.reset(**kwargs)
            
        processed_frame = preprocess(obs)

        self.stacked_state = np.stack([processed_frame] * self.stack_frames, axis=0)
    ...
    def step(self, action):
        ...
        self.stacked_state[-1] = processed_frame
        reward = total_reward
