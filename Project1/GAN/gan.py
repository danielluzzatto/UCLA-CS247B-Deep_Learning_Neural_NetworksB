...
def discriminator_loss(logits_real: Tensor, logits_fake: Tensor) -> Tensor:
    ...
    real_targets = torch.ones_like(logits_real)
    fake_targets = torch.zeros_like(logits_fake)
    
    loss_real = bce_loss(logits_real, real_targets)
    loss_fake = bce_loss(logits_fake, fake_targets)
...
def generator_loss(logits_fake: Tensor) -> Tensor:
    ...
    targets = torch.ones_like(logits_fake)
...
def ls_discriminator_loss(scores_real: Tensor, scores_fake: Tensor) -> Tensor:
    ...
    real_targets = torch.ones_like(scores_real)
    fake_targets = torch.zeros_like(scores_fake)
    loss = 0.5 * torch.mean((scores_real - real_targets) ** 2) + 0.5 * torch.mean((scores_fake - fake_targets) ** 2)
...
def ls_generator_loss(scores_fake: Tensor) -> Tensor:
    ...
    targets = torch.ones_like(scores_fake)
    loss = 0.5 * torch.mean((scores_fake - targets) ** 2)
