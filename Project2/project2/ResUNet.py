...
class ConditionalUnet(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        down1 = self.fusion1(down1, temb2, cemb2)      # FusionBlock handles spatial broadcast

        down2 = self.downblock2(down1)
        down2 = self.fusion2(down2, temb1, cemb1)      # same here

        latentvec = self.to_vec(down2)
        up0 = self.upblock0(latentvec)

        up1 = self.upblock1(up0, down2)                # UnetUp cats internally, don't pre-cat
        up1 = self.fusion3(up1, temb2, cemb2)

        up2 = self.upblock2(up1, down1)                # same
